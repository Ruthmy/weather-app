/* eslint-disable no-unused-vars */
import React from 'react';
import { NavLink } from 'react-router-dom';
import { useSelector } from 'react-redux';

import continentsImages from '../assets/bd/continentsImages';
import regionListArray from '../assets/bd/regionList';

import '../styles/Home.css';

const Home = () => {
  // Get the continents from the global state
  const continents = useSelector((state) => state.location.location);

  return (
    <>
      <div className="home d-flex-row">
        <img
          className="map"
          src="https://upload.wikimedia.org/wikipedia/commons/thumb/8/80/World_map_-_low_resolution.svg/1024px-World_map_-_low_resolution.svg.png"
          alt="Mapa Mundi"
        />
        <div className="home__info">
          <h1 className="title">Countries of the World</h1>
          <p className="description">
            Click on a continent to see the countries of that continent.
          </p>
        </div>
      </div>
      <div className="continents">
        {
        // In case continents are empty (API bug, etc.)
        // continents are rendered from the regionListArray variable
          continents
            ? regionListArray && regionListArray.map((region) => (
              <NavLink
                to={`/${region.EnglishName}`}
                className="continents__link"
                key={region.EnglishName}
              >
                <img
                  className="continents__image"
                  src={continentsImages[region.EnglishName]}
                  alt={region.EnglishName}
                />
              </NavLink>
            ))
            : continents && continents.map((continent) => (
              <NavLink
                to={`/${continent.EnglishName}`}
                className="continents__link"
                key={continent.EnglishName}
              >
                <img
                  className="continents__image"
                  src={continentsImages[continent.EnglishName]}
                  alt={continent.EnglishName}
                />
              </NavLink>
            ))
        }
      </div>
    </>
  );
};

export default Home;
